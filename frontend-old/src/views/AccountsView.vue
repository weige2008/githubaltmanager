...
const guideSteps = [
  {
    ...
    desc: '点击下方按钮跳转到 GitHub 的 Token 创建页面（需先登录 GitHub）。URL 已预选大部分权限，但企业/Copilot/项目等新权限需在下一步用脚本补勾。',
  ...
  {
    title: '一键全选所有权限',
    desc: '在上一步打开的 GitHub 页面上，按 F12 打开开发者工具，切到 Console（控制台），粘贴下面这行代码并回车，它会自动勾选页面上所有权限复选框（包括 URL 无法预选的企业/Copilot/项目等权限）：',
    link: '',
    linkText: '',
    code: 'document.querySelectorAll(\'input[type="checkbox"][name="scopes[]"]\').forEach(c=>{if(!c.checked)c.click()});console.log("✅ 已勾选全部权限")'
  },
  {
    title: '设置参数并生成',
    desc: '在 Note 填写备注（如 GAM），Expiration 选择有效期（推荐 No expiration），滚动确认所有权限已勾选，然后点击底部绿色按钮「Generate token」。',
    link: '',
    linkText: '',
